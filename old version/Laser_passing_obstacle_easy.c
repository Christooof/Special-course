...
int main() {
    ...
    char* fwd_mrc = "drive @v0.3 :($irdistfrontmiddle<0.5)\n";
    //char* fwd_mrc = "drive @v0.3 :($l4<0.5)\n";
    ...
    if (lmssrv.config) {
        ...
        while (1) {
            ...
            if (l4<0.7 && help==0){
               ...
               send(client_fd_mrc, "turn -90\n", strlen("turn -90\n"), 0);
               send(client_fd_mrc, "fwd 0.8\n", strlen("fwd 0.8\n"), 0);
               send(client_fd_mrc, "turn 90\n", strlen("turn 90\n"), 0);
               send(client_fd_mrc, "fwd 1.6\n", strlen("fwd 1.6\n"), 0);
               send(client_fd_mrc, "turn 90\n", strlen("turn 90\n"), 0);
               send(client_fd_mrc, "fwd 0.8\n", strlen("fwd 0.8\n"), 0);
               send(client_fd_mrc, "turn -90\n", strlen("turn -90\n"), 0);
               send(client_fd_mrc, "fwd 1\n", strlen("fwd 1\n"), 0);
               send(client_fd_mrc, "stop\n", strlen("stop\n"), 0);
               //close(client_fd_mrc);
